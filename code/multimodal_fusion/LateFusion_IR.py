...
for i in range(0,len(cor_label_test)):
    ...
    if cor_label_test[i]==1:
        ...
        a_elem.append(a[i])
        #---------hyper-parameter settings--------------------------
        t_ = np.power(np.array(t_elem), 0.54).astype(np.float32)
        v_ = np.power(np.array(v_elem), 0.43).astype(np.float32)
        a_ = np.power(np.array(a_elem), 0.03).astype(np.float32)
        #------------------------------------------
        decision_f = t_ * v_ * a_ 
        decision_label= decision_f.argmax(axis=1)
        #print(decision_label)
